<template>
  <div class="pagination">
    <span v-if="current > 1" class="item">
        <router-link :to="indexPath(current - 1)" v-html="prevText" class="link"></router-link>
    </span>
    <ul class="numList" v-if="total <= pageRange">
      <li class="item" v-for="index in total">
        <router-link :to="indexPath(index)" :class="['link',index===current ? 'active' : '']">{{index}}</router-link>
      </li>
    </ul>
    <ul class="numList" v-else>
      <li v-if="current - l > 1" class="item">
        <router-link :to="indexPath(1)" :class="['link',1===current ? 'active' : '']">1</router-link>
      </li>
      ...
      <li v-if="current + r < total" class="item">
        <router-link :to="indexPath(total)" :class="['link',total===current ? 'active' : '']">{{total}}</router-link>
      </li>
    </ul>
    <span v-if="current < total" class="item">
      <router-link :to="indexPath(current + 1)" v-html="nextText" class="link"></router-link>
    </span>
  </div>
</template>
